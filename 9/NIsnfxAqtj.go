...
func Make(set func(*foo,*foo, byte), list *foo){
	list.prev = list;
	list.next = list;
}
func Insert(get func(*foo, byte) (**foo), set func(*foo,*foo, byte), list *foo, elm *foo){
	elm.prev = list;
	elm.next = list.next;
	list.next = elm;
	elm.next.prev = elm;
...
func Remove(get func(*foo, byte) (**foo), set func(*foo,*foo, byte), elm *foo){
...
func InsertList(get func(*foo, byte) (**foo), set func(*foo,*foo, byte), list *foo, other *foo){
	if (Empty(get, other)) {
		return;
}
